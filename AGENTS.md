...
- **Standings fallbacks require an expected scope.** The default World Cup adapter advertises the bundled A–L group set; custom-competition adapters leave the scope open. A group-specific read falls back only when its requested group was declared expected; an absent group under an open scope means "no such group" and must never inject World Cup teams. An all-groups read falls back to the complete static roster when an expected table is absent — `StandingsResult` has one result-level `degraded`/`source` verdict and cannot honestly represent mixed live and fallback provenance.
