...
- **A rejection says which KIND it is.** `ParseResult` distinguishes `valid` / `definitive-none` / `malformed` / `ambiguous` / `unresolved`, and the line for remembering one is whether we UNDERSTOOD the payload. `valid`, `definitive-none` and `ambiguous` are conclusions drawn from bytes we read, and all three are stable across a refetch, so all three may be cached for a TTL. `malformed` and `unresolved` are facts about US — a shape we could not read, a clock that ran out — and remembering either would suppress the retry that recovers. (Getting this backwards cost a permanent per-command refetch loop: an ambiguity never converges if you re-ask it, and under the enrichment deadline the doomed fixtures starve the resolvable ones.)
