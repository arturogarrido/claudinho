...
- **A rejection says which KIND it is.** `ParseResult` distinguishes `valid` / `definitive-none` / `malformed` / `ambiguous` / `unresolved`, and only the first two may be cached. Recording "we could not read this" as "there is nothing here" suppresses the retry that would have recovered — the mirror image of never caching a transient error as a real negative.
